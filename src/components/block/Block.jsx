...
import { Card, FormatLargeNumber, Link, Loading, TooltipIcon, Flex } from '@thyme/core';
import {
  unix_to_short_date,
  hex_to_array,
  arr_to_hex,
...
export default function Block() {
  ...
  const poolReward = mojo_to_thyme(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_thyme(calculateBaseFarmerReward(blockRecord.height));

  const thymeFees = blockRecord.fees
    ? mojo_to_thyme(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link target="_blank" href={`https://www.thymeexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}>
    ...
    },
    {
      ...
      value: (
        <Link target="_blank" href={`https://www.thymeexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}>
    ...
    },
    {
      ...
      value: thymeFees ? `${thymeFees} ${currencyCode}` : '',
      tooltip: (
        <Trans>
    ...
    >
      <Card
        title={(
          <BlockTitle>
            <Trans>
              Block at height {blockRecord.height} in the Thyme
              blockchain
            </Trans>
          </BlockTitle>
        )}
