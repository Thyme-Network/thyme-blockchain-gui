...
import { AlertDialog } from '@thyme/core';
import {
  Typography,
  Button,
  Box,
...
import { thyme_to_mojo } from '../../../util/thyme';
...
export const CreateRLAdminWallet = () => {
  ...
  function create() {
    ...
    if (
      thymeper_input.value === '' ||
      Number(thymeper_input.value) === 0 ||
      !Number(thymeper_input.value) ||
      isNaN(Number(thymeper_input.value))
    ) {
      dispatch(
        openDialog(
          <AlertDialog>
    ...
    const thymeper = thyme_to_mojo(thymeper_input.value);
    const thymeper_value = Number.parseInt(Number(thymeper));
    ...
    // var fee = thyme_to_mojo(fee_input.value);
