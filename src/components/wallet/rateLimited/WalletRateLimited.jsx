...
import { AlertDialog, Card, Flex } from '@thyme/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const thymeper_input = ip_parsed.limit;
    ...
    const thymeper_value = Number.parseInt(Number(thymeper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (thyme per interval):{' '}
                  {mojo_to_thyme_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
