...
import thymeEnvironment from '../util/thymeEnvironment';
import thymeConfig from '../util/config';
...
if (!handleSquirrelEvent()) {
  ...
  const ensureCorrectEnvironment = () => {
    ...
    if (!thymeEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
  ...
  if (ensureSingleInstance() && ensureCorrectEnvironment()) {
    ...
    thymeConfig.loadConfig('mainnet');
    ...
    const createWindow = () => {
      ...
      if (!thymeConfig.manageDaemonLifetime()) {
      ...
      mainWindow.on('close', (e) => {
        ...
        if (decidedToClose || !thymeConfig.manageDaemonLifetime()) {
          return;
        }
        e.preventDefault();
        if (!isClosing) {
    ...
    const appReady = async () => {
      ...
      // if the daemon isn't local we aren't going to try to start/stop it
      if (thymeConfig.manageDaemonLifetime()) {
        thymeEnvironment.startThymeDaemon();
      }
    };
  ...
  const getMenuTemplate = () => {
    const template = [
      ...
      },
      {
        ...
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'Thyme Blockchain Wiki' }),
            click: () => {
              openExternal(
                'https://github.com/Thyme-Network/thyme-blockchain/wiki',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Thyme-Network/thyme-blockchain/wiki/FAQ',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Thyme-Network/thyme-blockchain/releases',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Thyme-Network/thyme-blockchain/blob/master/CONTRIBUTING.md',
              );
            },
          },
          {
          ...
          },
          {
            ...
            click: () => {
              openExternal(
                'https://github.com/Thyme-Network/thyme-blockchain/issues',
              );
            },
          },
          {
            ...
            click: () => {
              openExternal('https://keybase.io/team/thyme_network.public');
            },
          },
          {
            label: i18n._(/* i18n */ { id: 'Follow on Twitter' }),
            click: () => {
              openExternal('https://twitter.com/thyme_project');
            },
          },
        ],
      },
    ...
    if (process.platform === 'darwin') {
      // Thyme Blockchain menu (Mac)
      template.unshift({
        label: i18n._(/* i18n */ { id: 'Thyme' }),
        submenu: [
          {
            label: i18n._(/* i18n */ { id: 'About Thyme Blockchain' }),
            click: () => {
              openAbout();
            },
          },
