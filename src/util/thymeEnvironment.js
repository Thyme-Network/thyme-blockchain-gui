...
const getThymeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('thyme');
  // first see if we can get a thyme exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or thyme is on the path
    try {
      version = child_process
